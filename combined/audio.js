...
// Invert Signal for Sidechain
    ...
    function audioSetup() {
        ...
        const maxChannelCount = Tone.getContext().rawContext.destination.maxChannelCount;
...
// let midiInput, midiOutput, midiMsg = {}
       // setupMidi(midiDeviceIn, midiDeviceOut) // deviceIn, deviceOut
        ...
        for (let i = 0; i < soundsFiles.length; i++) {
            ...
            sliderDomObject = audioTrack[i].slider
    ...
    function updateBroadcastChannel(i) {
        if (audioTrack[i].scene > 1) {
            let max = 0;
            sliderNosByScenes[audioTrack[i].scene].forEach(
                (index) => {
                    max = Math.max(audioTrack[index].slider.value, max)
                })

            //Broadcast the Values to the visual thingie
            bc.postMessage(audioTrack[i].scene + ":" + max);
            setSceneSlider(audioTrack[i].scene, max)
        }
    }
    ...
    function controlChange(control) {
        ...
        currentSlider = audioTrack[controllerNo]
        if (currentSlider) {
    ...
    function highlightSelectedSlider(sceneNo, sliderNoInScene) {
        ...
        newSlider = audioTrack[newSliderNo]
